...
\@ref(fig:Nobel) appeared in @national1975science 
...
```{r cap2, echo=FALSE}
cap2 <- "The black line shows numbers of US Nobel prizes, for given time intervals.
The blue dots show average number per year, as indicated by the axis on the right."
```
...
gph1 <- xyplot(sin((1:50)*0.92) ~ I((1:50)*0.92), xlab="", ylab="",
               aspect="xy",
       main=list("A: Pattern is hidden to view", 
                 x=0.1, y=-0.5, just='left', font=1))
gph2 <- xyplot(sin((1:50)*0.92) ~ I((1:50)*0.92), xlab="", ylab="",
       main=list("B: Pattern is now obvious", 
                 x=0.1, y=-0.5, just='left', font=1))
print(gph1, position=c(0,0,0.52,1), more=TRUE)
print(gph2, position=c(0.48,0,1,1))
```
